        ...
        <div className="relative h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center pb-20 pt-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 sm:gap-0">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-1">
                  <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight leading-none truncate">{ticket.id}</h1>
                  <span className={`px-2.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest border shrink-0 ${getPriorityColor(ticket.priority)}`}>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <button className="flex-1 sm:flex-initial px-4 sm:px-5 py-3 bg-white/5 hover:bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
                <Edit2 size={14} /> <span className="sm:inline">Edit</span>
              </button>
              <button className="flex-1 sm:flex-initial px-4 sm:px-6 py-3 bg-primary text-white rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-primary/40 hover:bg-primary/90 transition-all flex items-center justify-center gap-2 active:scale-95 font-bold">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-3xl sm:rounded-[2.5rem] p-5 sm:p-8 border border-white shadow-xl shadow-slate-200/40">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                <Shield size={14} className="text-primary" /> Issue Description
              </h3>
            ...
            <div className="bg-white rounded-3xl sm:rounded-[2.5rem] p-5 sm:p-8 border border-white shadow-xl shadow-slate-200/40">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 flex items-center gap-3">
                <MessageSquare size={14} className="text-primary" /> Activity Stream
              </h3>
              ...
              <div className="space-y-8 relative before:absolute before:left-5 sm:before:left-6 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                {ticket.messages.map((msg, idx) => (
                  <div key={idx} className={`relative pl-12 sm:pl-14`}>
                    ...
                    <div className={`p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] border transition-all ${
                      msg.role === 'admin' 
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 sm:mb-3 gap-1 sm:gap-0">
                        <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest truncate">
                          {msg.sender} {msg.role === 'admin' && <span className="text-primary ml-1">• Administrator</span>}
                        </span>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-3xl sm:rounded-[2.5rem] p-5 sm:p-8 border border-white shadow-xl shadow-slate-200/40 overflow-hidden relative group">
              ...
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                <User size={14} className="text-primary" /> User context
              </h3>
              
              <div className="flex flex-col items-center text-center mb-6 sm:mb-8">
                <Avatar name={ticket.user} size="xl" className="sm:size-2xl mb-4 ring-8 ring-slate-50" />
                <h4 className="text-lg sm:text-xl font-black text-slate-900 leading-none mb-1">{ticket.user}</h4>
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="p-3 sm:p-4 bg-slate-50 rounded-2xl sm:rounded-3xl border border-slate-100 hover:bg-white hover:border-primary hover:shadow-lg transition-all cursor-pointer group">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-0.5 sm:mb-1">Email link</p>
                ...
                <div className="p-3 sm:p-4 bg-slate-50 rounded-2xl sm:rounded-3xl border border-slate-100 hover:bg-white hover:border-amber-400 hover:shadow-lg transition-all cursor-pointer group">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-0.5 sm:mb-1">Phone stream</p>
            ...
            <div className="bg-white rounded-3xl sm:rounded-[2.5rem] p-5 sm:p-8 border border-white shadow-xl shadow-slate-200/40">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                <Clock size={14} className="text-primary" /> Information Vault
              </h3>
              
              <div className="space-y-6">
                {[
                  { label: 'Submission date', value: ticket.date, icon: Calendar, color: 'blue' },
                  { label: 'Current status', value: ticket.status, icon: () => getStatusIcon(ticket.status), color: 'amber' },
                  { label: 'Assigned category', value: ticket.category, icon: Shield, color: 'purple' },
                  { label: 'Wait time', value: '2 hours', icon: Clock, color: 'rose' },
                ].map((item, idx) => (
